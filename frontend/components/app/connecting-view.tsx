...
export const ConnectingView = React.forwardRef<HTMLDivElement, ConnectingViewProps>((props, ref) => {
  return (
    <div ref={ref} className="fixed inset-0 z-40 flex flex-col items-center justify-center p-6 bg-gradient-to-br from-slate-100 via-amber-50/70 to-slate-200 text-slate-900">
      ...
      <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-amber-300 bg-white/90 px-4 py-1.5 text-xs font-mono font-bold tracking-widest text-amber-900 uppercase shadow-md">
      ...
      <div className="mt-8 flex flex-col items-center gap-3">
        <div className="flex items-center gap-3 rounded-full border border-slate-200 bg-white/90 px-6 py-2.5 shadow-xl text-sm font-semibold text-slate-800">
          <Loader2 className="h-5 w-5 animate-spin text-amber-600" />
          <span>Joining emergency channel... Please wait on the line.</span>
        </div>

        <p className="text-xs text-slate-600 max-w-sm text-center font-medium">
